...
module.exports = (_opts) => {
  ...
  _opts.servers.forEach((server) => {
    ...
    const apiVersion = server.apiVersion || _opts.apiVersion;

    const defaultOpts = {
      baseUrl: server.baseUrl + '/api/' + apiVersion,
      headers: {}
    };
    ...
    for (let key in api) {
      ...
      if (typeof fx === 'function') {
        ...
        instance[key] = fx.factory(_opts.promisify, requestWithDefaultOpts);
        if (fx.alias) {
