  ...
  static _getGizmoText() {
    ...
    this._calls.forEach((item, idx, array) => {
      if (Array.isArray(item)) {
        let params;
        if (['V', 'Vertex', 'Is', 'Tag', 'As'].indexOf(array[idx - 1]) !== -1) {
          if (item[0]) {
        ...
        } else if (['Out', 'In', 'Both', 'LabelContext', 'Intersect', 'And', 'Union', 'Or', 'Except', 'Difference'].indexOf(array[idx - 1]) !== -1) {
          if (item[0] instanceof this.constructor) {
            params = this.constructor._getGizmoText.apply(item[0]);
            if (item[1]) {
              params += (',' + JSON.stringify(item[1]));
            }
          } else {
  ...
  static _generateFuncs4Query() {
    ...
    const generator = (funcName) => {
      this[funcName] = (...args) => {
        ...
        if (this._promisify) {
          return CommonImport.Promise.promisify(this._query).call(this, gizmoText);
        } else {
